...
lnames = load(file = "02-networkConstruction.RData");
...
geneOverlap_ToFile <- function(prefix, geneOverlapOjb){
	...
	top3GeneOverlap = positionGeneList[order(sapply(positionGeneList, function(x) x[1], simplify=TRUE), decreasing=TRUE)][0:3]
	
	for (v in top3GeneOverlap)
	{
		pValue = v[[1]]
		idx = v[[2]]
...
topGO_GeneEnrichment <- function (bg_genes, geneOverlapOjb)
{
	positionGeneList = geneOverlapOjb$positionList
	modulePairList = geneOverlapOjb$modulePairList
	unionListGeneList = geneOverlapOjb$unionList
